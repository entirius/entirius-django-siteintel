...
- `run_source(audit_id, source, run_id)`: a previous run's source task (or retry) never writes to a rerun;
  swept stuck audits send `report_ready` once; a refused rerun leaves the caller's `Audit` instance unchanged.
