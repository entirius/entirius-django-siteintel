...
def finish_audit(audit_id: str, attempt: int = 0) -> None:
    ...
    if report_service.finish(audit_id):
        return
    interval = siteintel_settings.value("SITEINTEL_URLSCAN_POLL_INTERVAL_S")
    if attempt * interval <= siteintel_settings.value("SITEINTEL_URLSCAN_POLL_BUDGET_S") + interval:
        finish_audit.apply_async((audit_id, attempt + 1), countdown=interval)
        return
