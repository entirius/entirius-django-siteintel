...
- `services/audit_service` — `request_audit(*, domain_or_url, channel_idx, requested_by)` reuses the newest valid
  audit of the domain (signal at once, no fetch) or creates one with a pending report per source; `rerun_audit`,
  `expire_audits(now=None)`, `run_now` (development). `report_service` runs/polls/fails reports and finishes the run.
...
- `tasks/` (queue `siteintel_default`): `run_audit` → chord of `run_source` (retries only `upstream`, 3×, backoff)
  ...
  countdown until `SITEINTEL_URLSCAN_POLL_BUDGET_S`; `expire_audits`. Never `time.sleep` (a test greps `src/`).
...
→ 201 new / 200 reused, `POST audits/<uuid>/rerun/` → 202. `ENVIRONMENT == "development"` only (else 404):
